...
import { sendEmailNotification, sendActiveSaleEmailNotification, sendCorrectionRequiredEmailNotification } from '../services/emailService.js';
...
export const upload = multer({
  ...
  fileFilter: (req, file, cb) => {
    if (file.size > 1 * 1024 * 1024) {
      req.fileValidationError = 'El archivo excede el límite permitido de 1MB';
      return cb(null, false, new Error('El archivo excede el límite permitido de 1MB'));
    }
    cb(null, true);
  }
...
export const createSale = async (req, res) => {
  // Primero, manejamos el posible error de Multer
  ...
  try {
    ...
    const otherImages = req.files && req.files.other_images 
      ? req.files.other_images.map(file => file.path) 
      : null;

    // Verifica si hay archivos y si alguno excede el límite
    if (req.files && req.files.other_images) {
      const oversizedFiles = req.files.other_images.filter(file => file.size > 1 * 1024 * 1024);
      if (oversizedFiles.length > 0) {
        return res.status(400).json({ 
          message: 'Uno o más archivos exceden el límite de 1MB permitido' 
        });
      }
    }
    ...
    const saleData = {
      ...
      other_images: otherImages ? otherImages.join(',') : null,
      is_priority: 0,
  ...
  try {
    if (req.query.export === 'true') {
      ...
      const salesExport = sales.map(sale => {
        ...
        let executiveName = 'No asignado';
        if (sale.executive && sale.executive.first_name && sale.executive.last_name) {
          executiveName = `${sale.executive.first_name} ${sale.executive.last_name}`;
        }
...
export const updateSale = async (req, res) => {
  try {
    ...
    // Nuevo bloque para enviar correo si el estado de la venta se actualiza a 4
    if (Number(filteredUpdatedSaleData.sale_status_id) === 4) {
      // Obtener el historial de la venta para encontrar quién la ingresó
      const saleHistory = await SaleHistory.findOne({
        where: {
          sale_id: sale.sale_id,
      ...
      if (saleHistory) {
        ...
        if (saleCreator && saleCreator.email) {
          try {
            await sendCorrectionRequiredEmailNotification(sale, saleCreator, req.body);
          } catch (emailError) {
            console.error(`Error al enviar el correo a ${saleCreator.first_name} ${saleCreator.last_name}:`, emailError);
          }
        }
